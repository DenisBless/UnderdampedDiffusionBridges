...
class Phi4Distr(Target):
    def __init__(
            ...
            lambd: float = 0.022,
            # lat_shape: int = 2,
            sample_bounds=None, can_sample=True, log_Z=None,
            **kwargs
    ):
        ...
        dim = prod
        if len(lat_shape) != 2:
            raise ValueError(f"The lattice configuration has an invalid shape {len(lat_shape)} instead of 2.\n "
    ...
    def log_prob(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        return -jnp.sum(action, axis=-1, keepdims=True)
    ...
    def visualise(self, samples: chex.Array = None, axes=None, show=False, prefix='') -> dict:
        ...
        ax.hist(magnetization, bins=jnp.linspace(-6,6,100), density=True)
        plt.xlim([-6, 6])
        # plt.savefig(project_path('./figures/phi4.pdf'), bbox_inches='tight', pad_inches=0.1)
        # plt.show()
