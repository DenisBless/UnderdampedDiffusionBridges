...
def init_dbs_ud(key, cfg, target):
    ...
    params = {'params': {'betas': jnp.ones((alg_cfg.num_steps,)),
                         ...
                         'mass_std': jnp.ones(dim) * inverse_softplus(1.),
                         'dt': init_dt(cfg),
    ...
    def backward_model(step, x, vel, model_state, params, aux):
        ...
        return model_state.apply_fn[1](model_params, input, step,
                                       jax.lax.stop_gradient(langevin_vals))

    def drift_fn(step, x, params):
        if alg_cfg.drift == 'anneal':
            return langevin_fn(step, x, params)
        elif alg_cfg.drift == 'zero':
            return jnp.zeros_like(x), None
        elif alg_cfg.drift == 'target':
            target_score = jax.grad(lambda x: jnp.squeeze(target.log_prob(x)))(x)
            return target_score, None
        else:
            raise ValueError(f'Unknown drift {alg_cfg.drift}')

    key, key_gen = jax.random.split(key)
    model_state = init_model(key, params, cfg, dim, learn_forward=True, learn_backward=True)
